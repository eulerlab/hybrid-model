...
def loss_SemiSL(recon_x, x, alpha1, alpha2, beta1, alpha_Wsc, alpha_Wtc, beta_Wf,\
    ...
    #
    l2temp=0.0
    for temp in alpha_Wsc: # conv3d (spatial conv)
        l2temp = l2temp+ temp.norm(2) # + temp.norm(2)
    ...
    tempB_z, tempC_z, tempH_z, tempW_z =z.size()
    ...
    #
    loss_ae=(MSE + L2loss_z + L2loss_z2 + L1loss_z)/(tempB_z* tempC_z* tempH_z* tempW_z) # /(tempB_z* tempC_z* tempH_z* tempW_z*tempD_z)
...
class GaussianNoise(nn.Module):
    ...
    def forward(self, x):
        if self.training and self.sigma != 0:
            ...
            #sampled_noise = torch.randn(x.size(),requires_grad=False).to(device)* self.sigma
            x = x + sampled_noise # x + sampled_noise # x + x*sampled_noise
        return x
